...
# =========================================
# ✅ SALSIFY IMAGES - SIMPLE & RELIABLE
# =========================================
def get_salsify_images(url):
    """
    Extract Salsify images from rendered HTML.
    Looks for images in noscript tags (lazy loading fallback).
    Returns images in order they appear on page.
    """
    html = get_html(url)
    images = []
    seen_urls = set()
    ...
    try:
        ...
        # Find all asset containers (each image property)
        ...
        for idx, container in enumerate(asset_containers):
            ...
            print(f"   Container {idx + 1}: {prop_name}")
            
            # Find noscript (contains fallback image URL)
            noscript = container.find("noscript")
            if not noscript:
                print(f"      ⚠️ No noscript found")
                continue
            
            # Find img tag in noscript
            img_tag = noscript.find("img")
            if not img_tag:
                print(f"      ⚠️ No img tag in noscript")
                continue
            ...
            if srcset:
                ...
                # Extract highest quality (last URL)
                urls = [u.strip().split()[0] for u in srcset.split(",") if u.strip()]
                img_url = urls[-1] if urls else None
                print(f"      ✅ Found in srcset: {img_url[:60]}...")
            elif src:
                img_url = src
                print(f"      ✅ Found in src: {img_url[:60]}...")
            
            # Add if valid and not duplicate
            if img_url and "salsify" in img_url:
