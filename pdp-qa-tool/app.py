...
# =========================================
# WALGREENS SALSIFY COPY + IMAGE HOTFIX
# =========================================
def _wg_salsify_norm_key(value):
    try:
        return _normalize_salsify_property_key(value)
    except Exception:
        value = normalize_salsify_asset_name(value)
        value = value.replace(" ", "_")
        return value.strip("_")
...
def _wg_salsify_decode_value(value):
    value = str(value or "")
    if not value:
        return ""
    try:
...
def _wg_collect_salsify_property_values(html_text, *property_names):
    html_text = str(html_text or "")
    if not html_text:
        return []
    ...
    def add(value):
        clean_value = _wg_salsify_decode_value(value)
        if clean_value and clean_value not in seen:
            seen.add(clean_value)
            out.append(clean_value)

    visible_map = extract_salsify_visible_property_map(html_text) or {}
    prop_map = (visible_map.get("properties", {}) or {}) if isinstance(visible_map, dict) else {}

    for prop_name in property_names:
        norm_key = _wg_salsify_norm_key(prop_name)
        for value in prop_map.get(norm_key, []) or []:
            add(value)

    raw_html = html.unescape(html_text)
    for prop_name in property_names:
        escaped_name = re.escape(str(prop_name or ""))
        patterns = [
            rf'"property"\s*:\s*"{escaped_name}"[^{{}}]{{0,2500}}?"value"\s*:\s*"((?:\\.|[^"\\])*)"',
            rf'"name"\s*:\s*"{escaped_name}"[^{{}}]{{0,2500}}?"value"\s*:\s*"((?:\\.|[^"\\])*)"',
...
def _build_walgreens_general_salsify_override_from_html(html_text):
    title = first_non_placeholder_copy_value(*_wg_collect_salsify_property_values(
        html_text,
        "General Product Title",
        "General Title",
        "Product Title",
    ))
    description = first_non_placeholder_copy_value(*_wg_collect_salsify_property_values(
        html_text,
        "General Description",
    ...
    gathered_features = []
    for i in range(1, 6):
        values = _wg_collect_salsify_property_values(
            html_text,
            f"General Feature {i}",
            f"General Feature{i}",
            f"General Bullet {i}",
        ...
        slot_value = first_non_placeholder_copy_value(*values)
        if slot_value:
            feature_slots[i] = slot_value
            gathered_features.append(slot_value)

    gathered_features = normalize_salsify_feature_values(dedupe_preserve_order(gathered_features), max_features=5)
...
def _wg_blank_slot(name="missing"):
    try:
        return make_blank_salsify_image_slot(name)
    except Exception:
        return {"name": str(name or "missing"), "url": ""}


def _wg_salsify_image_key(img):
    if not isinstance(img, dict):
        return ""
    return str(img.get("url", "") or "").strip()


def _wg_find_salsify_image(images, *queries):
    query_tokens = [normalize_salsify_asset_name(q) for q in queries if normalize_salsify_asset_name(q)]
    for token in query_tokens:
        for img in list(images or []):
            if not isinstance(img, dict):
                continue
            name = normalize_salsify_asset_name(img.get("name", ""))
            if name and (token == name or token in name):
                return img
    return None
...
def reorder_walgreens_salsify_images_for_visual(images, max_slots=6):
    ...
    def add(img, blank_name=""):
        if not isinstance(img, dict):
            if blank_name:
                ordered.append(_wg_blank_slot(blank_name))
            return False
        key = _wg_salsify_image_key(img)
        if not key or key in used:
            if blank_name:
                ordered.append(_wg_blank_slot(blank_name))
            return False
    ...
    online_img = _wg_find_salsify_image(
        imgs,
        "online optimized image",
        "online optimized image-",
        "online image",
    ...
    ingredient_img = _wg_find_salsify_image(
        imgs,
        "ingredient label image",
        "ingredient label",
        "ingredient",
    )
    io_img = _wg_find_salsify_image(
        imgs,
        "atf i/o generic",
        "atf i o generic",
        "atf io generic",
...
_original_get_salsify_bundle = get_salsify_bundle

def get_salsify_bundle(url):
    bundle = _original_get_salsify_bundle(url)
    try:
        html_text = get_html(url)
        wg_override = _build_walgreens_general_salsify_override_from_html(html_text)
        text_bundle = dict((bundle or {}).get("text", {}) or {})
        retailer_overrides = dict(text_bundle.get("retailer_overrides", {}) or {})
        retailer_overrides["walgreens"] = wg_override
        text_bundle["retailer_overrides"] = retailer_overrides
        bundle["text"] = text_bundle
    except Exception:
        pass
    return bundle
...
def apply_retailer_salsify_image_limits(retailer_name, images):
    retailer = str(retailer_name or "").strip().lower()
    if retailer == "walgreens":
        max_images = int(get_retailer_salsify_requirements(retailer_name).get("max_images", 6) or 6)
        return reorder_walgreens_salsify_images_for_visual(images, max_slots=max(0, min(max_images, MAX_IMAGE_SLOTS_TO_COMPARE)))
    return _original_apply_retailer_salsify_image_limits(retailer_name, images)
...
def finalize_salsify_copy_for_retailer(retailer_name, s_text):
    retailer = str(retailer_name or "").strip().lower()
    if retailer != "walgreens":
        return _original_finalize_salsify_copy_for_retailer(retailer_name, s_text)

    out = dict(s_text or {})
    retailer_overrides = out.get("retailer_overrides", {}) or {}
    wg_override = retailer_overrides.get("walgreens", {}) or {}

    selected_title = first_non_placeholder_copy_value(wg_override.get("title", ""), out.get("title", ""))
    selected_description = first_non_placeholder_copy_value(wg_override.get("description", ""), out.get("description", ""))

    selected_features = []
    feature_slots = wg_override.get("feature_slots", {}) or {}
    for i in range(1, 6):
        slot_value = first_non_placeholder_copy_value(feature_slots.get(i, ""))
        if slot_value:
            selected_features.append(slot_value)

    if not selected_features:
        selected_features = normalize_salsify_feature_values(wg_override.get("features", []) or [], max_features=5)
    ...
    if not selected_features:
        ...
        for value in out.get("features", []) or []:
            ...
            if value:
                fallback_candidates.append(value)
        selected_features = normalize_salsify_feature_values(dedupe_preserve_order(fallback_candidates), max_features=5)

    out["title"] = selected_title
    out["description"] = selected_description
    out["features"] = selected_features[:5]
    for i in range(1, 11):
        out[f"feature{i}"] = selected_features[i - 1] if i <= 5 and i - 1 < len(selected_features) else ""
    return out
