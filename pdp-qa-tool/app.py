...
st.title("PDP QA Tool v4.4 — CVS Details Area Debugger")
st.caption("Focused on raw details area. Exports the exact details block, the details block before WHAT'S INCLUDED, the WHAT'S INCLUDED block, heading-only features, and vendor raw refs.")
...
def get_html(url):
    ...
    try:
        r = requests.get(url, headers={"User-Agent": "Mozilla/5.0", "Connection": "keep-alive"}, timeout=18)
...
def has_nav_junk(text):
    ...
    junk = [
        'skip to main content','cvs pharmacy','weekly ad','extra big deals','manage prescriptions','schedule a vaccine','photo coupons',
        'carepass','sign in account','use the cvs app','how to get it','rating & reviews','extrabucks rewards','search cvs',
...
def is_eligibility_or_rebate(text):
    txt = clean_text(text).lower()
    flags = ['hsa/fsa', 'purchase by', 'postmarked', 'original receipt', 'restrictions apply', 'check with your provider']
...
def cleanup_frontmatter(text):
    ...
    txt = re.sub(r'^[A-Z0-9][A-Za-z0-9\-\+&/,()\'’ ]{6,160}\s+\d+\s*(?:Ct|Sht|Boxes?|Rolls?|Packs?|CT)\b[^A-Z]{0,25}', '', txt)
...
def get_nextjs_chunks(html_text):
    matches = re.findall(r'self\.__next_f\.push\(\[1,(.*?)\]\)', html_text, re.DOTALL)
    chunks = []
    for m in matches:
        try:
            t = m.strip()
...
def around(text, marker, back=350, forward=4000):
...
def find_vendor_objects(raw_text, data_map):
    ...
    for b,p,source,preview in candidates:
        ...
        out.append({
            ...
            'preview': preview,
            'resolved_bullets_raw': json.dumps(bullets, ensure_ascii=False)[:6000] if bullets is not None else '',
            'resolved_para_raw': str(para)[:6000] if para is not None else '',
        })
    return out

# ==============================
# Visible sections + details area parsing
# ==============================
...
def extract_sections(visible_text, title):
    ...
    for i,line in enumerate(lines):
        ...
        if title_idx is None and title and clean_text(line).lower()==clean_text(title).lower():
            title_idx=i+1
        if whats_idx is None and ("what's included" in low or 'what’s included' in low or ':' in line and len(line)<120 and line.upper()==line):
            # broad marker for block-like heading lines
            if "what's included" in low or 'what’s included' in low or re.search(r'[A-Z][A-Z0-9\'’/&\- ]{5,}:', line):
                whats_idx=i
...
def split_details_area(details_text):
    ...
    # explicit what's included onward.
    start_idx = None
    ...
    if start_idx is not None:
        headings_block = clean_text(txt[start_idx:])
    else:
        # fallback to first ALL CAPS dash/colon heading.
        m2 = re.search(r'([A-Z][A-Z0-9\'’/&\- ]{4,}\s*[—:-].+)$', txt_clean, re.S)
        if m2:
            headings_block = clean_text(m2.group(1))
    whats_only = ''
    m3 = re.search(r'(WHAT\'?S INCLUDED\s*[—:-].+)$', txt_clean, re.S)
    if m3:
        whats_only = clean_text(m3.group(1))
    elif re.search(r'WHAT’S INCLUDED\s*[—:-].+', txt_clean, re.S):
        m4 = re.search(r'(WHAT’S INCLUDED\s*[—:-].+)$', txt_clean, re.S)
        whats_only = clean_text(m4.group(1)) if m4 else ''
    # heading-only lines.
    heading_lines = []
    for m5 in re.finditer(r'([A-Z0-9][A-Z0-9\'’/&\- ]{3,60})\s*[—:-]\s*(.+?)(?=(?:[A-Z0-9][A-Z0-9\'’/&\- ]{3,60}\s*[—:-])|$)', headings_block or txt_clean, re.S):
        heading_lines.append(f"{clean_text(m5.group(1))} — {clean_text(m5.group(2))}")
    heading_only = ' | '.join(clean_feature_list(heading_lines, keep_eligibility=True))
    sentence_only = ' | '.join(clean_feature_list([s for s in split_sentences(txt_clean) if len(s)>=40], keep_eligibility=True))
    return txt_clean, prose, headings_block, whats_only, heading_only, sentence_only

# ==============================
# Description/feature pickers
# ==============================
def choose_desc(theories):
    priority=['vendor_desc_1','vendor_desc_2','vendor_desc_3','details_prose','visible_details_3sent','visible_item_3sent','jsonld_desc','meta_desc']
    for key in priority:
...
def choose_feat(feat_theories):
    priority=['details_heading_lines_clean','details_whats_clean','details_heading_lines_rawkeep','details_sentences_clean','vendor_features_1']
    for key in priority:
...
def process_row(row):
    ...
    feat_theories={
        ...
        'details_heading_lines_clean': clean_feature_list(details_heading_only.split(' | ') if details_heading_only else []),
        'details_heading_lines_rawkeep': clean_feature_list(details_heading_only.split(' | ') if details_heading_only else [], keep_eligibility=True),
        ...
        'details_sentences_clean': clean_feature_list(item_sentence_only.split(' | ') if False else details_sentence_only.split(' | '), keep_eligibility=False),
    }
    ...
    strategy={
        ...
        'feat_details_heading_lines_clean': ' | '.join(feat_theories['details_heading_lines_clean']),
        'feat_details_heading_lines_rawkeep': ' | '.join(feat_theories['details_heading_lines_rawkeep']),
        'feat_details_whats_clean': ' | '.join(feat_theories['details_whats_clean']),
        'feat_details_sentences_clean': ' | '.join(feat_theories['details_sentences_clean']),
    }
    ...
    raw_debug={
        ...
        'raw_vendor_desc_window': raw_vendor_desc[:6000],
        'raw_vendor_feature_window': raw_vendor_feat[:6000],
        'raw_details_window': raw_details[:6000],
        'raw_item_window': raw_item[:6000],
        'raw_whats_window': raw_whats[:6000],
        'raw_title_window': raw_title[:6000],
        'visible_details_section_raw': sections['details'][:7000],
        'visible_item_section_raw': sections['item'][:7000],
        'visible_whats_section_raw': sections['whats'][:7000],
        'visible_title_section_raw': sections['title_section'][:7000],
        'details_cleaned_block': details_clean[:7000],
        'details_prose_block': details_prose[:7000],
        'details_headings_block': details_headings_block[:7000],
        'details_whats_only_block': details_whats_only[:7000],
        'details_heading_only_lines': details_heading_only[:7000],
        'details_sentence_only_lines': details_sentence_only[:7000],
    }
    ...
    vendor_debug={
        ...
        'vendor_1_preview': vendor_objects[0]['preview'][:6000] if len(vendor_objects)>0 else '',
...
# helper uses extraction defined above
def extract_whats_features(section):
    ...
    # add heading lines too
    vals.extend([f for f in clean_feature_list([x for x in re.split(r'\s*\|\s*', txt) if x], keep_eligibility=True)])
    return vals
...
if uploaded_file:
    ...
    df.rename(columns={
        'salsify url':'salsify_url','retail url':'retail_url','sku id':'sku','product sku':'sku','cvs rpc':'cvs_rpc','title':'salsify_title','salsify title':'salsify_title'
    }, inplace=True)
    ...
    if st.button('Run CVS details area debug v4.4'):
        progress=st.progress(0)
        status=st.empty()
        summary_rows=[]; strategy_rows=[]; raw_rows=[]; vendor_rows=[]; marker_rows=[]
        total=len(df)
        with ThreadPoolExecutor(max_workers=8) as ex:
            ...
            for i,fut in enumerate(as_completed(futures), start=1):
                try:
                    ...
                    summary_rows.append(s); strategy_rows.append(stg); raw_rows.append(r); vendor_rows.append(v); marker_rows.append(m)
                except Exception:
                    pass
                progress.progress(i/total)
                status.write(f'Processed {i}/{total}')
        if summary_rows:
            with pd.ExcelWriter('pdp_qa_tool_v4_4_output.xlsx', engine='openpyxl') as writer:
            ...
            with open('pdp_qa_tool_v4_4_output.xlsx','rb') as f:
                st.success('Done.')
                st.download_button('Download v4.4 Excel output', data=f, file_name='pdp_qa_tool_v4_4_output.xlsx', mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
